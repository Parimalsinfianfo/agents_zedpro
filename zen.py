...
def init_company_data():
    ...
    if cursor.fetchone()[0] == 0:
        sample_data = [
            ("services", "What services do you offer?", "We offer AI solutions, web development, mobile app development, digital marketing, cloud services, automation solutions, and education technology services.", "services, offerings, solutions"),
            ...
            ("education", "What education solutions do you offer?", "We develop AI-powered learning platforms, educational chatbots, student analytics systems, and personalized learning solutions for educational institutions.", "education, learning, edtech, schools")
        ]
...
def update_analytics():
    ...
    education_leads = cursor.execute("SELECT COUNT(*) FROM visitors WHERE project_interest = 'Education Solutions'").fetchone()[0]
    
    cursor.execute('''INSERT OR REPLACE INTO analytics (date, visitors_count, leads_generated, emails_sent, conversions)
                     VALUES (?, ?, ?, ?, ?)''', (today, visitors_today, leads_today, emails_today, education_leads))
    conn.commit()
    conn.close()
...
.education-highlight {
    background: linear-gradient(120deg, #2c3e50 0%, #4a235a 100%);
    padding: 1.5rem;
    border-radius: 12px;
    margin: 1.5rem 0;
...
# -----------------------------
# Add logo to sidebar
st.sidebar.image("https://github.com/Parimalsinfianfo/zedpro/blob/main/apprent.ai.png?raw=true", width=200)
...
if page == "🏠 Home - Chat Assistant":
    ...
    <div class="education-highlight">
        <div style="display: flex; align-items: center;">
            <div style="flex: 1;">
                <h2 style="color: #e0e0ff; margin-top: 0;">Transform Education with AI</h2>
                <p style="font-size: 1.1rem;">Our cutting-edge AI solutions are revolutionizing learning experiences for educational institutions, students, and educators.</p>
    ...
    with col1:
        ...
        with st.form("visitor_form"):
            ...
            project_type = st.selectbox("Project Interest", [
                ...
                "Digital Marketing", "Cloud Services", "Education Solutions", "Consulting"
            ])
    ...
    with col2:
        ...
        <div class="education-card">
            ...
            <h3 style="color: #e0e0ff; margin-top: 0;">Education Solutions</h3>
            <p>Transform learning experiences with our AI-powered education technology:</p>
            <ul>
            ...
            <button style="background: var(--primary); color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: 600; margin-top: 1rem; cursor: pointer;">Learn More</button>
        </div>
        """, unsafe_allow_html=True)
...
# -----------------------------
    ...
    with col4:
        education_leads = pd.read_sql_query("SELECT COUNT(*) as count FROM visitors WHERE project_interest = 'Education Solutions'", conn).iloc[0]['count']
        st.metric("Education Leads", education_leads)
    ...
    st.markdown('<div class="section-title"><h3>🎓 Education Sector Analytics</h3></div>', unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    
    with col1:
        edu_leads = pd.read_sql_query("""SELECT DATE(visit_time) as date, COUNT(*) as count 
                                        FROM visitors 
                                        WHERE project_interest = 'Education Solutions'
                                        AND visit_time >= date('now', '-30 days')
                                        GROUP BY DATE(visit_time) 
                                        ORDER BY date""", conn)
        if not edu_leads.empty:
            fig = px.bar(edu_leads, x='date', y='count', title='Education Leads (Last 30 Days)',
                         template="plotly_dark")
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("No education sector data available")
...
# -----------------------------
    ...
    with col3:
        project_filter = st.selectbox("Filter by Project Interest", ["All", "AI Solutions", "Web Development", 
                                                                    ...
                                                                    "Cloud Services", "Education Solutions", 
                                                                    "Consulting"])
                ...
                with col2:
                    ...
                    if st.button(f"Send Follow-up Email", key=f"email_{lead['id']}"):
                        ...
                        <body style="font-family: Arial, sans-serif;">
                            ...
                            <p>I wanted to follow up on your recent inquiry about our {lead['project_interest']} services at ZedPro Digital.</p>
        ...
        if not leads_df.empty:
            ...
            with col1:
                campaign_type = st.selectbox("Campaign Type", [
                    "Newsletter", "Product Update", "Festival Greeting", "Follow-up", "Education Update", "Custom"
                ])
                
                recipient_type = st.selectbox("Send to", [
                    "All Leads", "New Leads Only", "Qualified Leads", "Education Leads", "Custom Selection"
                ])
...
<p>Best regards,<br>ZedPro Digital Team</p>""")
            ...
            if st.button("Send Campaign", type="primary"):
                if subject and email_content:
                    ...
                    elif recipient_type == "Education Leads":
                        recipients = pd.read_sql_query(
                            "SELECT name, email FROM visitors WHERE project_interest = 'Education Solutions'", conn
                        )
                    else:
...
<p>Warm wishes,<br>ZedPro Digital Team</p>""",
            ...
            "Education Update": """<h2>Education Technology Update - {name}</h2>
<p>Discover how ZedPro Digital is transforming education with AI:</p>
<ul>
    <li>Personalized learning platforms for schools</li>
    <li>AI-powered tutoring systems</li>
    <li>Student performance analytics</li>
    <li>Automated assessment tools</li>
</ul>
<p>Schedule a demo to see how our solutions can benefit your institution!</p>
<p>Best regards,<br>ZedPro Digital Team</p>""",
...
# -----------------------------
    ...
    with tab1:
        ...
        # Education knowledge highlight
        st.markdown("""
        <div class="education-card" style="margin-bottom: 2rem;">
            <h3 style="color: #e0e0ff; margin-top: 0;">🎓 Education Solutions Knowledge</h3>
            <p>Key information about our education technology offerings:</p>
            <ul>
                <li>AI-powered personalized learning platforms</li>
                <li>Intelligent tutoring systems with natural language processing</li>
                <li>Student performance analytics dashboards</li>
                <li>Automated assessment and feedback systems</li>
                <li>Learning management system integrations</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
